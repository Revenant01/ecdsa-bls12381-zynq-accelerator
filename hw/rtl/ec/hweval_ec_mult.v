...
module hweval_ec_mult(
    ...
    // Instantiating ec_multiplier module
    ec_mult #(
        .WIDTH_p(381),
        .WIDTH_k(256)
    ) dut (
    ...
    always @(posedge clk) begin
        ...
        if (!resetn) begin
            ...
            // Initialize point P (using simple test values - point at infinity initially)
            i_Xp     <= 381'b0;
            i_Yp     <= 381'b1;
            i_Zp     <= 381'b0;
            
            // Initialize scalar k (small value for testing)
            i_k      <= 256'd2;
            ...
            if (state == 2'b00) begin
                ...
                // Update inputs based on results (XOR pattern for continuous testing)
                i_p      <= i_p;
                i_Xp     <= o_Xr[380:0] ^ i_k[255:0];
                i_Yp     <= o_Yr[380:0];
                i_Zp     <= o_Zr[380:0];
                i_k      <= i_k + 256'd1;  // Increment scalar for next iteration
